...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    _player!.stream.error.listen((e) {
      if (mounted) setState(() => _playerError = e);
    });
    _player!.stream.playing.listen((p) {
      if (p && mounted) setState(() => _playerLoading = false);
    });
    _init();
  ...
  Future<void> _play(Channel c) async {
    ...
    try {
      await _player!.open(Media(c.url), play: true);
    } catch (e) {
      setState(() { _playerError = e.toString(); _playerLoading = false; });
    }
  }
  ...
  Future<void> _playDirect() async {
    ...
    await _play(Channel(name: 'Flux direct', url: url, group: 'Direct'));
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      _banner(),
      if (_current != null || _playerError != null) _playerBox(),
      _tabs(),
      if (_tab == 0) _urlRow(),
    ...
    child: Stack(children: [
      ...
      if (_playerError != null)
        Center(child: Padding(padding: const EdgeInsets.all(16),
          child: Text('Erreur : $_playerError',
  ...
  Widget _tile(Channel c) {
    final fav = _isFav(c);
    final isCurrent = _current?.url == c.url;
    return Container(
      color: isCurrent ? const Color(0xFF1B2338) : Colors.transparent,
      child: ListTile(
        dense: true,
